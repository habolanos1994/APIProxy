...
async function logs(type, message, event) {
  ...
  parentPort.on('message', (message) => {
    ...
    if (message.type == 'restart' && message.serviceName === ServiceName) {
...
const server = net.createServer((socket) => {
    ...
    socket.on('data', async (data) => {
        ...
        try {
            ...
            handleSocketError(clientAddress, error, socket);
        }
    });
    ...
    socket.on('error', (err) => {
        handleSocketError(clientAddress, err, socket);
    });
});
...
async function processSingleRequest(socket, clientAddress, requestData) {
    ...
    const response = await ApiCall(clientAddress, requestData, APImode);
    if (!response.success) {
...
function handleSocketError(clientAddress, error, socket) {
    console.error(`Socket error for ${clientAddress}: ${error.message}`);
    socket.write('FAIL');

    const errorData = {
        serial: error.serial || 'Unknown',
